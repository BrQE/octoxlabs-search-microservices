...
class Transmitter:
    ...
    def send_to_elasticsearch(self, ch, method, properties, body):
        ...
        try:
            ...
            logger.info(
                f"Processing message - Query: {message['query']}, User: {message['username']}, IP: {message['ip']}"
            )
            ...
            response = self.es.index(index=settings.ES_LOG_INDEX_NAME, body=document)
            logger.info(f"Successfully indexed document with ID: {response['_id']}")

            # Acknowledge message
            ch.basic_ack(delivery_tag=method.delivery_tag)
            logger.debug(
                f"Acknowledged message with delivery tag: {method.delivery_tag}"
            )
