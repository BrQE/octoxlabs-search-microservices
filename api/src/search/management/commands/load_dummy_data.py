...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # Sample data
        dummy_data = [
            {"Hostname": "octoxlabs01", "Ip": ["192.168.1.101", "10.0.0.101"]},
            {"Hostname": "octoxlabs02", "Ip": ["192.168.1.102"]},
            {"Hostname": "octoxlabs-prod01", "Ip": ["10.0.1.101", "10.0.2.101"]},
            {"Hostname": "octoxlabs-prod02", "Ip": ["10.0.1.102"]},
            {"Hostname": "octoxlabs-dev01", "Ip": ["172.16.1.101"]},
            {"Hostname": "octoxlabs-staging01", "Ip": ["172.16.2.101", "172.16.2.102"]},
        ]
        ...
        try:
            ...
            if es.indices.exists(index=settings.ELASTICSEARCH_INDEX):
                self.stdout.write(
                    f"Deleting existing index '{settings.ELASTICSEARCH_INDEX}'..."
                )
                es.indices.delete(index=settings.ELASTICSEARCH_INDEX)

            # Create new index with mapping
            self.stdout.write(
                f"Creating index '{settings.ELASTICSEARCH_INDEX}' with mapping..."
            )
